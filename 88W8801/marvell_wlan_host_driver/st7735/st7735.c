...
void lcdClear(void)
{
	...
	for (uint16_t i = 0, j; i < LCD_X; ++i)
		for (j = 0; j < LCD_Y; ++j)
			lcdWriteData16(g_u16BackColor);
	CS_SET;
}

void lcdInitTable(const uint8_t *init_lcd)
{
	uint8_t length;
	while (*init_lcd != 0)
	{
		lcdWriteCommand(*init_lcd++);
		length = *init_lcd++;
		while (length != 0)
		{
			--length;
			lcdWriteData8(*init_lcd++);
		}
	}
}

static const uint8_t init_lcd_1[] = {
...
static const uint8_t init_lcd_2[] = {
	0xE0, 16,
	...
	0x2A, 4, 0x00, 0x02, 0x00, 0x82,
	0x2B, 4, 0x00, 0x03, 0x00, 0x83,
	0xF0, 1, 0x01,
	0xF6, 1, 0x00,
	0x3A, 1, 0x05,
	0x29, 0,
...
{
	g_u16ForeColor = u16ForeColor;
}

void lcdSetBackColor(uint16_t u16BackColor)
{
	g_u16BackColor = u16BackColor;
}
...
{
	if (ax > bx)
		ax ^ bx && (bx ^= ax ^= bx, ax ^= bx);
	if (ay > by)
		ay ^ by && (by ^= ay ^= by, ay ^= by);
	--bx, --by;
	uint16_t pos = ay;
	if (bFill)
	{
...
void lcdDrawInt8(uint16_t x, uint16_t y, int8_t i8Data)
{
	...
	do
	{
		*(pu8Order + i8Data) = *pu8Order % 10;
		*pu8Order /= 10;
	} while (*pu8Order && --i8Data);

	while (i8Data < 3)
		lcdDrawChar(x += 8, y, *(pu8Order + i8Data++) + '0');
}
...
void lcdDrawUInt8(uint16_t x, uint16_t y, uint8_t u8Data)
{
	...
	do
	{
		*(pu8Order + u8Data) = *pu8Order % 10;
		*pu8Order /= 10;
	} while (*pu8Order && --u8Data);

	while (u8Data < 3)
		lcdDrawChar(x += 8, y, *(pu8Order + u8Data++) + '0');
}
...
void lcdDrawInt16(uint16_t x, uint16_t y, int16_t i16Data)
{
	...
	do
	{
		*(pu16Order + i16Data) = *pu16Order % 10;
		*pu16Order /= 10;
	} while (*pu16Order && --i16Data);

	while (i16Data < 5)
		lcdDrawChar(x += 8, y, *(pu16Order + i16Data++) + '0');
}
...
void lcdDrawUInt16(uint16_t x, uint16_t y, uint16_t u16Data)
{
	...
	do
	{
		*(pu16Order + u16Data) = *pu16Order % 10;
		*pu16Order /= 10;
	} while (*pu16Order && --u16Data);

	while (u16Data < 5)
		lcdDrawChar(x += 8, y, *(pu16Order + u16Data++) + '0');
}
...
void lcdSetImageScale(lcdMeasurement *lcdmImage)
{
	...
	if (!g_lcdmImage.u16Width)
		++g_lcdmImage.u16Width;
	...
	if (!g_lcdmImage.u16Height)
		++g_lcdmImage.u16Height;
...
void lcdDrawImage(const uint8_t *pu8Data)
{
	...
	lcdInitArea(
		g_lcdmImage.x,
		g_lcdmImage.y,
		g_lcdmImage.x + g_lcdmImage.u16ZoomWidth - 1,
		g_lcdmImage.y + g_lcdmImage.u16ZoomHeight - 1
	);
	const uint8_t *pos;
