 ...
 * Parameter: PDN_GPIO_Port(IN) -> GPIO port where PDN is located
		PDN_Pin(IN) -> PDN pin
...
{
	/* Use the PDN pin to reset the chip */
	...
	/* Enable SDIO mode and DMA at the same time,
		because writing to the same register
		requires at least 3*SDIO_CK+2*PCLK2 intervals
	*/
	/* In addition, if you keep enabling DMA in CMD53, DMA will lock */
	SDIO->DCTRL |= SDIO_DCTRL_SDIOEN | SDIO_DCTRL_DMAEN;

	/* Send CMD5 and set the OCR register (3.2-3.4V) */
	uint32_t cmd_resp;
	uint8_t err;
	if ((err = sdio_cmd5(0, &cmd_resp, SDIO_RETRY_MAX)) ||
		(err = sdio_cmd5(0x300000, &cmd_resp, SDIO_RETRY_MAX))
		)
		return err;
	/* Parse R4 and get the Func number */
	sdio_core.func_total_num = FUNC_NUM_IN_R4(cmd_resp);

	uint8_t index;
	for (index = SDIO_FUNC_0; index < sdio_core.func_total_num; ++index)
		(sdio_core.func + index)->func_num = index;

	/* Send CMD3 to get the address and R6 */
	if ((err = sdio_cmd3(0, &cmd_resp)))
		return err;

	/* Parse RCA and send CMD7 */
	if ((err = sdio_cmd7(RCA_IN_R6(cmd_resp) << 16, &cmd_resp)))
		return err;

	/* Get CCCR version and SDIO version */
	if ((err = sdio_get_cccr_version(&sdio_core.cccr_version)) ||
		(err = sdio_get_sdio_version(&sdio_core.sdio_version))
		)
		return err;

	/* Switched to 4-bit bus, 24MHz clock */
	if ((err = sdio_set_bus_width(SDIO_BUS_WIDTH_4)))
		return err;
	sdioStruct.BusWide = SDIO_BUS_WIDE_4B;
	sdioStruct.ClockDiv = SDIO_CLK_24MHZ;
	SDIO_Init(SDIO, sdioStruct);

	/* Parse the CIS pointer of each Func */
	for (index = SDIO_FUNC_0; index < sdio_core.func_total_num; ++index)
		if ((err = sdio_get_cis_ptr(index, &cmd_resp)) ||
			(err = sdio_cis_parse(index, cmd_resp))
			)
			return err;

	/* Enable Func */
	for (index = SDIO_FUNC_1; index < sdio_core.func_total_num; ++index)
		if ((err = sdio_enable_func(index)))
			return err;

	/* Enable interrupts */
	if ((err = sdio_enable_mgr_int()))
		return err;
	for (index = SDIO_FUNC_1; index < sdio_core.func_total_num; ++index)
		if ((err = sdio_enable_func_int(index)))
			return err;

	/* Set the chunk size */
	for (index = SDIO_FUNC_1; index < sdio_core.func_total_num; ++index)
		if ((err = sdio_set_blk_size(index, SDIO_BLK_SIZE)))
			return err;

	return SDIO_ERR_OK;
}

/* Function name: sdio_get_cccr_version
 * Parameter: cccr_version(OUT) -> CCCR version
 * Return value: Returns the execution result
 * Description: Get the CCCR version by reading the CCCR register address 0x0
		|-7-|-6-|-5-|-4-|-3-|-2-|-1-|-0-|
...
{
	if (!cccr_version)
		return SDIO_ERR_INVALID_PARA;
	/* Read CCCR0 */
	uint8_t version;
	if (sdio_cmd52(SDIO_EXCU_READ, SDIO_FUNC_0, SDIO_CCCR_SDIO_VERSION, 0, &version))
		return SDIO_ERR_GET_VER_FAILED;
	*cccr_version = version & 0xF;
	return SDIO_ERR_OK;
}

/* Function name: sdio_get_sdio_version
 * Parameter: sdio_version(OUT) -> SDIO version
 * Return value: Returns the execution result
 * Description: Get the SDIO version by reading the CCCR register address 0x0
		|-7-|-6-|-5-|-4-|-3-|-2-|-1-|-0-|
		|--SDIO version-|-CCCR version--|
...
/* Function name: sdio_enable_func
 * Parameter: func_num(IN) -> Func number
 * Return value: Returns the execution result
 * Description: Enable the specified Func according to the Func number
 */
uint8_t sdio_enable_func(uint8_t func_num)
{
	if (func_num >= SDIO_FUNC_NUM)
		return SDIO_ERR_INVALID_PARA;
	/* Modify CCCR2 */
	uint8_t enable;
	if (sdio_cmd52(SDIO_EXCU_READ, SDIO_FUNC_0, SDIO_CCCR_IO_ENABLE, 0, &enable) ||
		sdio_cmd52(SDIO_EXCU_WRITE, SDIO_FUNC_0, SDIO_CCCR_IO_ENABLE, enable | 1 << func_num, NULL)
		)
		return SDIO_ERR_ENABLE_FUNC_FAILED;
	/* Waiting for Func to be ready */
	uint8_t ready = 0;
	while (!(ready & 1 << func_num))
		if (sdio_cmd52(SDIO_EXCU_READ, SDIO_FUNC_0, SDIO_CCCR_IO_READY, 0, &ready))
			return SDIO_ERR_ENABLE_FUNC_FAILED;
	/* Update Func status */
...
/* Function name: sdio_disable_func
 * Parameter: func_num(IN) -> Func number
 * Return value: Returns the execution result
 * Description: Disable the specified Func according to the Func number
 */
uint8_t sdio_disable_func(uint8_t func_num)
{
	...
	if (sdio_cmd52(SDIO_EXCU_READ, SDIO_FUNC_0, SDIO_CCCR_IO_ENABLE, 0, &enable) ||
		...
		return SDIO_ERR_DISABLE_FUNC_FAILED;
...
/* Function name: sdio_enable_func_int
 * Parameter: func_num(IN) -> Func number
 * Return value: Returns the execution result
 * Description: Enable the specified Func interrupt according to the Func number
 */
uint8_t sdio_enable_func_int(uint8_t func_num)
{
	...
	/* Modify CCCR4 */
	uint8_t enable;
	if (sdio_cmd52(SDIO_EXCU_READ, SDIO_FUNC_0, SDIO_CCCR_INT_ENABLE, 0, &enable) ||
		sdio_cmd52(SDIO_EXCU_WRITE, SDIO_FUNC_0, SDIO_CCCR_INT_ENABLE, enable | 1 << func_num, NULL)
		)
		return SDIO_ERR_ENABLE_FUNC_INT_FAILED;

	/* Update Func interrupt status */
	(sdio_core.func + func_num)->func_int_status = SDIO_ENABLE;
	return SDIO_ERR_OK;
}

/* Function name: sdio_disable_func_int
 * Parameter: func_num(IN) -> Func number
 * Return value: Returns the execution result
 * Description: Disable the specified Func interrupt according to the Func number
 */
uint8_t sdio_disable_func_int(uint8_t func_num)
{
	...
	if (sdio_cmd52(SDIO_EXCU_READ, SDIO_FUNC_0, SDIO_CCCR_INT_ENABLE, 0, &enable) ||
		...
		return SDIO_ERR_DISABLE_FUNC_INT_FAILED;
...
/* Function name: sdio_enable_mgr_int
 * Parameters: NULL
 * Return value: Returns the execution result
 * Description: Enable CCCR interrupt master switch
 */
uint8_t sdio_enable_mgr_int(void)
{
	/* Modify CCCR4, the interrupt switch is located at bit 0 */
	uint8_t enable;
	if (sdio_cmd52(SDIO_EXCU_READ, SDIO_FUNC_0, SDIO_CCCR_INT_ENABLE, 0, &enable) ||
		sdio_cmd52(SDIO_EXCU_WRITE, SDIO_FUNC_0, SDIO_CCCR_INT_ENABLE, enable | 0x1, NULL)
		)
		return SDIO_ERR_ENABLE_MGR_INT_FAILED;

	/* Update interrupt master switch status */
	sdio_core.sdio_int_mgr = SDIO_ENABLE;
	return SDIO_ERR_OK;
}

/* Function name: sdio_disable_mgr_int
 * Parameters: NULL
 * Return value: Returns the execution result
 * Description: Disable CCCR interrupt master switch
 */
uint8_t sdio_disable_mgr_int(void)
{
	...
	if (sdio_cmd52(SDIO_EXCU_READ, SDIO_FUNC_0, SDIO_CCCR_INT_ENABLE, 0, &enable) ||
		...
		return SDIO_ERR_DISABLE_MGR_INT_FAILED;
...
/* Function name: sdio_cmd53_write
 * Parameter: func_num(IN) -> Func number
		address(IN) -> address
		inc_addr(IN) -> Is the address cumulative?
		buf(IN) -> Write buffer
		size(IN) -> Write data length
		cur_blk_size(IN) -> Current Func block size
 * Return value: Returns the execution result
 * Description: Write CMD53 in blocks
 */
static uint8_t sdio_cmd53_write(
	uint8_t func_num,
	uint32_t address,
	uint8_t inc_addr,
	uint8_t *buf,
	...
	)
{
	...
	*/
	...
	/* OP Code: 1 address increment, 0 fixed address */
	sdioCmdStruct.Argument |= inc_addr ? 0x4000000 : 0x0;
	sdioCmdStruct.Argument |= address << 9;
	sdioCmdStruct.Argument |= size / cur_blk_size + (size % cur_blk_size ? 1 : 0);
	sdioCmdStruct.CmdIndex = SDIO_CMD53;
	...
	while (!(err || __SDIO_GET_FLAG(SDIO, SDIO_FLAG_CMDREND)))
		if (sdio_check_err())
			err = SDIO_ERR_CMD53_WRITE_FAILED;

	/* Clear command response flag */
	...
	while (!(err || __SDIO_GET_FLAG(SDIO, SDIO_FLAG_DATAEND)))
		if (sdio_check_err())
			err = SDIO_ERR_CMD53_WRITE_FAILED;

	/* Clear data transfer flag */
