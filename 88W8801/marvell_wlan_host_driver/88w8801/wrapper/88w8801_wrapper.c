...
uint8_t wrapper_init(
	core_err_e *status,
	wlan_cb_t *callback,
	GPIO_TypeDef *PDN_GPIO_Port, uint32_t PDN_Pin
	)
{
	sys_status = status;
...
{
	if (!((sys_now() & CHECK_TIMEOUTS_INTERVAL) != CHECK_TIMEOUTS_INTERVAL || *sys_status))
		sys_check_timeouts();
	return __SDIO_GET_FLAG(SDIO, SDIO_FLAG_SDIOIT)
		? (__SDIO_CLEAR_FLAG(SDIO, SDIO_FLAG_SDIOIT), wlan_process_packet())
		: CORE_ERR_OK;
}

#if LWIP_TCP
err_t wrapper_tcp_connect(
	struct tcp_pcb **pcb,
...
{
	tcp_close(*pcb), *pcb = NULL;
}

err_t wrapper_tcp_write(struct tcp_pcb **pcb, uint8_t *data_buf, uint16_t data_len)
{
	return tcp_write(*pcb, data_buf, data_len, TCP_WRITE_FLAG_COPY);
}
#endif
