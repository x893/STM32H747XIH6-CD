...
#if defined(USE_480x854)
	#include "../Components/mipi_lcd_480x854/eugen_mipi_lcd_480x854.h"
	#include "stm32h747i_eval_lcd_mipi_480x854.h"
#endif
#if defined(USE_480x800)
	#include "../Components/mipi_lcd_480x800/eugen_mipi_lcd_480x800.h"
	#include "stm32h747i_eval_lcd_mipi_480x800.h"
#endif
...
static LCD_LayerPropTypedef          layer_prop[GUI_NUM_LAYERS];
	...
	static DSI_VidCfgTypeDef hdsivideo_handle;
	/* Screen needed definitions */
	uint8_t pPage[]       = {0x00, 0x00, 0x02, 0x57}; /*   0 -> 599 */
	/* ZONES of the screen */
	uint8_t pCols[ZONES][4] =
	{
	#if (ZONES == 4 )
		{0x00, 0x00, 0x00, 0xFF}, /*   0 -> 255 */
		{0x01, 0x00, 0x01, 0xFF}, /* 200 -> 511 */
		{0x02, 0x00, 0x05, 0xFF}, /* 512 -> 599 */
		{0x06, 0x00, 0x03, 0xFF}, /* 600 -> 799 */
	#elif (ZONES == 2 )
		{0x00, 0x00, 0x01, 0xFF}, /*   0 -> 511 */
		{0x02, 0x00, 0x03, 0xFF}  
	#elif (ZONES == 1 )
		{0x00, 0x00, 0x03, 0xFF}, /*   0 -> 1023 */
	#endif  
	};
#endif
#if defined(USE_1280x720)
	static DSI_VidCfgTypeDef hdsivideo_handle;
	/* Screen needed definitions */
	uint8_t pPage[]       = {0x00, 0x00, 0x02, 0xCF}; /*   0 -> 719 */
	/* ZONES of the screen */
	uint8_t pCols[ZONES][4] =
	{
	#if (ZONES == 4 )
		{0x00, 0x00, 0x01, 0x3F}, /*   0 -> 319 */
		{0x01, 0x40, 0x02, 0x7F}, /* 320 -> 639 */
		{0x02, 0x80, 0x03, 0xBF}, /* 640 -> 959 */
		{0x03, 0xC0, 0x04, 0xFF}, /* 960 -> 1279*/
	#elif (ZONES == 2 )
		{0x00, 0x00, 0x02, 0x7F}, /*   0 -> 639 */
		{0x02, 0x80, 0x04, 0xFF}	/* 640 -> 1279*/
	#elif (ZONES == 1 )
		{0x00, 0x00, 0x04, 0xFF}, /*   0 -> 1279*/
	#endif  
	};
#endif
...
#if defined (USE_1280x720 ) | (USE_1024x600) | (USE_480x854) | (USE_480x800)
...
/*************************DSI Initialization***********************************/
  ...
  */
  ...
  VSA  = MIPI_LCD_480X854_VSYNC;        /* 1 */
  VBP  = MIPI_LCD_480X854_VBP;          /* 1 */
  VFP  = MIPI_LCD_480X854_VFP;          /* 1 */
  HSA  = MIPI_LCD_480X854_HSYNC;        /* 1 */
  HBP  = MIPI_LCD_480X854_HBP;          /* 1 */
  HFP  = MIPI_LCD_480X854_HFP;          /* 1 */
...
/***********************MIPI_LCD Initialization********************************/

  /* Initialize the MIPI_LCD LCD Display IC Driver
  *  depending on configuration set in 'hdsivideo_handle'.
  */
  Eugen_MIPI_480x854_Init(MIPI_LCD_FORMAT_RGB565, MIPI_LCD_ORIENTATION_PORTRAIT);
...
//  {
//    lcd_x_size = MIPI_LCD_720X1280_WIDTH;  /* 720 */
//    lcd_y_size = MIPI_LCD_720X1280_HEIGHT; /* 1280 */
//  }
//  else
//  {
//    /* lcd_orientation == LCD_ORIENTATION_LANDSCAPE */
//    lcd_x_size = MIPI_LCD_1280X720_WIDTH;  /* 1280 */
...
/***********************MIPI_LCD Initialization********************************/
  ...
  *  depending on configuration set in 'hdsivideo_handle'.
  */
  Eugen_MIPI_1280x720_Init(MIPI_LCD_FORMAT_RGB565, MIPI_LCD_ORIENTATION_PORTRAIT);
...
/*************************DSI Initialization***********************************/
  ...
  */
  ...
  /* The following values are same for portrait and landscape orientations */
  VSA  = MIPI_LCD_1024X600_VSYNC;        /* 10 */
  VBP  = MIPI_LCD_1024X600_VBP;          /* 15 */
  VFP  = MIPI_LCD_1024X600_VFP;          /* 16 */
  HSA  = MIPI_LCD_1024X600_HSYNC;        /* 2 */
  HBP  = MIPI_LCD_1024X600_HBP;          /* 20 */
  HFP  = MIPI_LCD_1024X600_HFP;          /* 20 */
...
/***********************MIPI_LCD Initialization********************************/
  ...
  *  depending on configuration set in 'hdsivideo_handle'.
  */
  Eugen_MIPI_1024x600_Init(MIPI_LCD_FORMAT_RGB565, MIPI_LCD_ORIENTATION_PORTRAIT);
...
{
#if defined(USE_1280x720)
	HAL_DSI_LongWrite(&hdsi, 0, DSI_DCS_LONG_PKT_WRITE, 4, MIPI_LCD_CMD_CASET, pCols[idx]);
#endif
#if defined(USE_800x480)
...
void LCD_ReqTear(void)
{
	...
	#elif (ZONES == 1 )
		uint16_t scanline = 719;
	#endif
  ScanLineParams[0] = scanline >> 8;
  ScanLineParams[1] = scanline & 0x00FF;
	HAL_DSI_LongWrite(&hdsi, 0, DSI_DCS_LONG_PKT_WRITE, 2, 0x44, ScanLineParams);     
	/* set_tear_on */
	HAL_DSI_ShortWrite(&hdsi, 0, DSI_DCS_SHORT_PKT_WRITE_P1, MIPI_LCD_CMD_TEEON, 0x00);
#endif
#if defined(USE_800x480)
	static uint8_t ScanLineParams[2];
	#if (ZONES == 4 )
...
void LCD_X_Config(void) 
{
  ...
  __HAL_DSI_WRAPPER_ENABLE(&hdsi);
#if defined(USE_1280x720)
	HAL_DSI_LongWrite(&hdsi, 0, DSI_DCS_LONG_PKT_WRITE, 4, MIPI_LCD_CMD_CASET, pCols[0]);
//  HAL_DSI_LongWrite(&hdsi, 0, DSI_DCS_LONG_PKT_WRITE, 4, MIPI_LCD_CMD_PASET, pPage);
